...
class JobControllerTest extends WebTestCase
{
    ...
    public function getMostRecentProgrammingJob()
    {
        ...
        $query = $em->createQuery('SELECT j from MaxJobeetBundle:Job j LEFT JOIN j.category c WHERE c.slug = :slug AND j.expires_at > :date ORDER BY j.created_at DESC');
    ...
    public function getExpiredJob()
    {
        ...
        $query = $em->createQuery('SELECT j from MaxJobeetBundle:Job j WHERE j.expires_at < :date');             
        $query->setParameter('date', date('Y-m-d H:i:s', time()));
        $query->setMaxResults(1);
...
//        $this->assertTrue($crawler->filter('.category_design .more_jobs')->count() == 0);
        ...
        $crawler = $client->request('GET', sprintf('/job/sMaxio-labs/paris-france/%d/web-developer', $this->getExpiredJob()->getId()));
        $this->assertTrue(404 === $client->getResponse()->getStatusCode());
    }}