<?php
namespace Max\JobeetBundle\Tests\Repository;
...
class JobRepositoryTest extends WebTestCase
{
    ...
    {
        $query = $this->em->createQuery('SELECT c FROM MaxJobeetBundle:Category c');
        $categories = $query->getResult();
 
        foreach($categories as $category) {
            $query = $this->em->createQuery('SELECT COUNT(j.id) FROM MaxJobeetBundle:Job j WHERE j.category = :category AND j.expires_at > :date');
            $query->setParameter('category', $category->getId());
            $query->setParameter('date', date('Y-m-d H:i:s', time()));
            $jobs_db = $query->getSingleScalarResult();
 
            $jobs_rep = $this->em->getRepository('MaxJobeetBundle:Job')->countActiveJobs($category->getId());
    ...
    public function testGetActiveJobs()
    {
        ...
        foreach ($categories as $category) {
            $query = $this->em->createQuery('SELECT COUNT(j.id) from MaxJobeetBundle:Job j WHERE j.expires_at > :date AND j.category = :category');
            $query->setParameter('date', date('Y-m-d H:i:s', time()));
            $query->setParameter('category', $category->getId());
            $jobs_db = $query->getSingleScalarResult();
 
            $jobs_rep = $this->em->getRepository('MaxJobeetBundle:Job')->getActiveJobs($category->getId(), null, null);
    ...
    public function testGetActiveJob()
    {
        ...
        $job_rep = $this->em->getRepository('MaxJobeetBundle:Job')->getActiveJob($job_db->getId());
        ...
        $job_rep = $this->em->getRepository('MaxJobeetBundle:Job')->getActiveJob($job_expired->getId());
